...
Location
- Project root: `d:/just files/Food Safety Subject/Project`
- Main code: `CookedFoodFreshness/`
...
$env:PYTHONPATH = "d:/just files/Food Safety Subject/Project"
...
-------------------------
Open PowerShell in your workspace root (the folder that contains `CookedFoodFreshness`). Example path in this project:

```powershell
cd "D:\just files\Food Safety Subject\Project"
```
...
---------------------------
1) Generate synthetic data (creates CSV):

```powershell
python "CookedFoodFreshness/backend/data_generation.py"
# Output: database/data/food_data.csv
```

2) Train the model (reads CSV, writes model and metrics):

```powershell
python "CookedFoodFreshness/backend/model_training.py"
# Output: models/freshness_model.pkl and reports/metrics.txt
```

3) Run the GUI (enter features and press Predict Freshness):
